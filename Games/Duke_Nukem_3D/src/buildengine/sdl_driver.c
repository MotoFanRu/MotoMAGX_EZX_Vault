...
#define	PEPC_VK_FX				(1<<15)	
#define	PEPC_VK_FY				(1<<14)	
...
#define PEPC_VK_STICK                           (1<<18)

const static unsigned short sdlSDL_gp2x_key[]= {
		SDLK_RETURN,
		SDLK_SPACE,
		SDLK_LEFT,
		SDLK_RIGHT,
...
const static unsigned long sdlSDL_gp2x_keygp2x[]= {
		PEPC_VK_START,
		PEPC_VK_SELECT,
		PEPC_VK_LEFT,
		PEPC_VK_RIGHT,
...
static int sdlSDL_gp2x_key_old_status[]= {
		SDL_RELEASED,
		SDL_RELEASED,
		SDL_RELEASED,
		SDL_RELEASED,
...
static void handle_events(void)
{
	...
	if ((key&PEPC_VK_UP_LEFT) && !((key&PEPC_VK_UP) || (key&PEPC_VK_LEFT))) {
	  key=key|PEPC_VK_UP;
	  key=key|PEPC_VK_LEFT;
	}
	if ((key&PEPC_VK_UP_RIGHT) && !((key&PEPC_VK_UP) || (key&PEPC_VK_RIGHT))) {
	  key=key|PEPC_VK_UP;
	  key=key|PEPC_VK_RIGHT;
	}
	if ((key&PEPC_VK_DOWN_LEFT) && !((key&PEPC_VK_DOWN) || (key&PEPC_VK_LEFT))) {
	  key=key|PEPC_VK_DOWN;
	  key=key|PEPC_VK_LEFT;
	}
	if ((key&PEPC_VK_DOWN_RIGHT) && !((key&PEPC_VK_DOWN) || (key&PEPC_VK_RIGHT))) {
	  key=key|PEPC_VK_DOWN;
	  key=key|PEPC_VK_RIGHT;
	}
	...
	for(i=0;i<15;i++)
	{
		...
		if (sdlSDL_gp2x_key_old_status[i]==SDL_RELEASED)
		{
